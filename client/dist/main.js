...
class SoundManager {
    ...
    baseVolume = 0.18; // Reduced from 0.4
    muffledVolume = 0.1;
    ...
    playEatSound() {
        if (!this.isInitialized || !this.eatSound || !this.audioContext)
            return;
        // Don't play if already playing
        if (this.eatSoundPlaying)
            return;
        this.ensureAudioContext();
    ...
    playDeathSound() {
        if (!this.isInitialized || !this.deathSound || !this.audioContext)
            return;
        this.ensureAudioContext();
        // Clone audio to allow overlapping sounds
        const soundClone = this.deathSound.cloneNode();
    ...
    setMenuMode(isMenuVisible, isDeathScreenVisible) {
        if (!this.isInitialized || !this.backgroundGain || !this.lowPassFilter)
            return;
    ...
    toggleMute() {
        if (!this.isInitialized || !this.backgroundGain)
            return false;
...
class WebGPURenderer {
    ...
    viewportWidth = 800;
    viewportHeight = 600;
    maxVertices = 500000; // Increased from 100k to prevent invisibility bugs
    constructor(canvas) {
        this.canvas = canvas;
    }
    async init() {
    ...
    createBindGroup() {
        if (!this.bindGroupLayout || !this.uniformBuffer)
            return;
    ...
    addQuad(vertices, x, y, w, h, r, g, b) {
    ...
    addTriangle(vertices, x1, y1, x2, y2, x3, y3, r, g, b) {
        vertices.push(x1, y1, r, g, b);
        vertices.push(x2, y2, r, g, b);
        vertices.push(x3, y3, r, g, b);
    }
    addTrapezoidHead(vertices, cx, cy, backWidth, frontWidth, length, angleRad, r, g, b) {
    ...
    addCircle(vertices, cx, cy, radius, r, g, b, segments = 16) {
    ...
    render() {
        ...
        this.device.queue.writeBuffer(this.uniformBuffer, 0, projection);
        const vertices = [];
        // Background
...
class Game {
    ...
    selectedColorIndex = Math.floor(Math.random() * COLORS.length);
    handleKeyDown(e) {
    ...
    setupCallbacks() {
        ...
        this.conn.db.bot_segment.onInsert((ctx, seg) => {
            ...
            if (!this.botSegments.has(botId))
                this.botSegments.set(botId, []);
        ...
        this.conn.db.player_died_event.onInsert((ctx, event) => {
            ...
            if (identity === this.myIdentity)
                this.showDeathScreen(event.killerName);
        });
    ...
    joinGame() {
        ...
        document.querySelectorAll('.color-option.selected').forEach(el => { selectedColor = el.dataset.color || COLORS[0]; });
        ...
        if (this.conn)
            this.conn.reducers.joinGame({ name, color: selectedColor });
    }
    startDirectionUpdateLoop() {
